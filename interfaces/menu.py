...
def _ui_main(page, *, lock_path: Path | None = None):
    ...
    def open_edit(index: int | None):
        ...
        df = ft.TextField(label="Description", value=g.get(
            "description", ""), width=400, multiline=True, min_lines=2)
        pf = ft.TextField(label="Prompt", value=g.get(
            "prompt", ""), width=400, multiline=True, min_lines=4)
        ...
        d = ft.AlertDialog(
            ...
            content=ft.Column(
                [nf, df, pf, ft.Row([ft.OutlinedButton("Cancel", on_click=lambda e: close()), ft.FilledButton(
                    "Save", on_click=save(index, nf, df, pf))], alignment=ft.MainAxisAlignment.END)],
                tight=True,
                spacing=12,
            ),
            modal=True,
    ...
    def open_settings(e=None):
        ...
        pf = ft.TextField(label="GEMINI_1PSID", value=settings.get("GEMINI_1PSID", ""),
                          width=450, password=True, can_reveal_password=True, disabled=use_browser_cookie)
        pt = ft.TextField(label="GEMINI_1PSIDTS", value=settings.get("GEMINI_1PSIDTS", ""),
                          width=450, password=True, can_reveal_password=True, disabled=use_browser_cookie)
        ...
        px = ft.TextField(label="Proxy (SOCKS5_PROXY)", value=settings.get(
            "SOCKS5_PROXY", ""), width=450, hint_text="e.g. socks5://127.0.0.1:8889")
        model_dd = ft.Dropdown(label="Model (Gemini)", value=settings.get(
            "MODEL", "unspecified"), width=450, options=[ft.dropdown.Option(m) for m in AVAILABLE_MODELS])
        ...
        om = ft.Dropdown(label="OPENAI_MODEL", value=settings.get("OPENAI_MODEL", "gpt-4o"),
                         width=360, options=[ft.dropdown.Option(settings.get("OPENAI_MODEL", "gpt-4o"))])
        ...
        async def refresh_models(e):
            ...
            try:
                ...
                refresh_btn.disabled = om.disabled  # respect section disabled state
                refresh_btn.update()
        ...
        openai_section = ft.Container(
            content=ft.Column(
                [ok, ou, ft.Row([om, refresh_btn], spacing=8)], spacing=12),
        ...
        gemini_btn = ft.ElevatedButton("Gemini (Unstable)", on_click=lambda e: switch_backend(
            ...
            ft.AlertDialog(
                ...
                content=ft.Column(
                    [
                        ...
                        ft.Text("Set GEMINI_1PSID to 'auto' to trigger browser login (may not work every time).",
